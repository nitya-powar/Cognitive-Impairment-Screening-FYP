...
All runs below use the same preprocessing cutoff: remove columns with `>=60%` missing values before modeling.
It also uses class weights- giving more weight to FN, so missing MCI is penalised
...
Use this table to choose the reduced-feature-set size for Random Forest. All runs use `14` fixed demographic/examination variables plus the stated number of top lab features.
...
Use this table to choose the reduced-feature-set size for XGBoost. All runs use `14` fixed demographic/examination variables plus the stated number of top lab features.
...
Use this table to compare subgroup fairness for the main baseline models before making any resampling decision. Enter paired values in the same cell as `B1 / B2`.
...
Use this section for experiments after the same targeted MCI oversampling rule was applied to `age 65-69`, `education 4`, and `education 5`. Rows added are `324` for all runs.

For the rerun resampling experiments below, class weights were removed (`1.0 / 1.0`) because MCI cases were already upweighted through targeted oversampling.
...
|---|---|---:|---|---|---|---|
| B3 | Yes | 0.207 | Yes |  |  | ECE=`0.0645`; wrongly overconfident predictions=`0.3%`; SHAP and LIME entries still need to be refreshed for `B3`. |
